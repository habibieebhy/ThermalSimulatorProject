"""Durable SQLite ledger for open research campaign jobs."""
...
CREATE INDEX IF NOT EXISTS campaign_jobs_status_idx
    ON campaign_jobs(status, submitted_at DESC);
CREATE INDEX IF NOT EXISTS campaign_jobs_parent_idx
    ON campaign_jobs(parent_job_id, submitted_at DESC);
"""
...
class CampaignJob:
    ...
    @classmethod
    def from_row(cls, row: sqlite3.Row) -> "CampaignJob":
        return cls(
            ...
            message=(
                str(row["message"]) if row["message"] is not None else None
            ),
...
class CampaignJobStore:
    ...
    @classmethod
    def from_settings(cls, settings: Settings) -> "CampaignJobStore":
        ...
        with closing(self.connect()) as connection:
            connection.execute(
                """INSERT INTO campaign_jobs (
                    ...
                    status, stage, progress, message, submitted_at, updated_at,
                    output_dir
    ...
    def get(self, job_id: str) -> CampaignJob:
        ...
        if row is None:
            raise KeyError(f"Unknown campaign job: {job_id}")
        return CampaignJob.from_row(row)

    def list(self, *, limit: int = 100) -> list[CampaignJob]:
        with closing(self.connect()) as connection:
        ...
        return [CampaignJob.from_row(row) for row in rows]

    def update_task_id(self, job_id: str, task_id: str) -> CampaignJob:
        with closing(self.connect()) as connection:
            connection.execute(
                """UPDATE campaign_jobs
                SET task_id = ?, updated_at = ? WHERE job_id = ?""",
        ...
        *,
        ...
        with closing(self.connect()) as connection:
            ...
            if row is None:
                connection.rollback()
                raise KeyError(f"Unknown campaign job: {job_id}")
            current = CampaignJob.from_row(row)
            ...
            stale = (
                ...
                and (now - lease_time).total_seconds()
                >= max(60, stale_after_seconds)
            )
            if (
                current.status == "running"
                and current.execution_token is not None
            ...
            connection.execute(
                ...
                SET task_id = ?, status = 'running', stage = 'initializing',
                    progress = ?, message = 'Initializing research services',
        ...
        *,
        ...
        resolved = max(
            ...
            min(
                ...
                else STAGE_PROGRESS.get(stage, 50),
            ),
        )
        now = utc_now().isoformat()
        token_clause = (
            "" if execution_token is None else " AND execution_token = ?"
        )
        ...
        with closing(self.connect()) as connection:
            connection.execute(
                ...
                SET status = 'completed', stage = 'completed', progress = 100,
                    message = 'Complete', completed_at = ?, updated_at = ?,
                    result_json_path = ?, summary_json = ?, error = NULL,
    ...
    def mark_failed(
        ...
        *,
        ...
        parameters: list[object] = [error, error, now, now, job_id]
        if execution_token is not None:
            parameters.append(execution_token)
        with closing(self.connect()) as connection:
            connection.execute(
